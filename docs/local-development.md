...
This guide shows how to iterate on the `worldbuild` CLI locally without publishing a release or committing to `main` first.
...
```bash
# From repo root
python -m src.worldbuild_cli --help
python -m src.worldbuild_cli init demo-project --ai claude --ignore-agent-tools --script sh
```

If you prefer invoking the script file style (uses shebang):

```bash
python src/worldbuild_cli/__init__.py init demo-project --script ps
```
...
# Now 'worldbuild' entrypoint is available
worldbuild --help
```
...
```bash
uvx --from . worldbuild init demo-uvx --ai copilot --ignore-agent-tools --script sh
```
...
```bash
python -c "import worldbuild_cli; print('Import OK')"
```
...
```bash
mkdir /tmp/worldbuild-test && cd /tmp/worldbuild-test
python -m src.worldbuild_cli init --here --ai claude --ignore-agent-tools --script sh  # if repo copied here
```
...
```bash
worldbuild check --skip-tls
worldbuild init demo --skip-tls --ai gemini --ignore-agent-tools --script ps
```
