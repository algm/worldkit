...
This guide will help you get started with World-Driven Fiction Writing using Worldbuilding Kit.

> NEW: All automation scripts now provide both Bash (`.sh`) and PowerShell (`.ps1`) variants. The `worldbuild` CLI auto-selects based on OS unless you pass `--script sh|ps`.
...
```bash
uvx --from git+https://github.com/algm/worldkit.git worldbuild init <PROJECT_NAME> --script ps  # Force PowerShell
