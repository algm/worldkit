...
def callback(ctx: typer.Context):
    ...
    if ctx.invoked_subcommand is None and "--help" not in sys.argv and "-h" not in sys.argv:
        show_banner()
        console.print(Align.center("[dim]Run 'worldbuild --help' for usage information[/dim]"))
        console.print()
...
def check_tool(tool: str, tracker: StepTracker = None) -> bool:
    ...
    # See: https://github.com/algm/worldkit/issues/123
...
def download_template_from_github(ai_assistant: str, download_dir: Path, *, script_type: str = "sh", verbose: bool = True, show_progress: bool = True, client: httpx.Client = None, debug: bool = False, github_token: str = None) -> Tuple[Path, dict]:
    ...
    pattern = f"worldbuilding-kit-template-{ai_assistant}-{script_type}"
    matching_assets = [
...
def ensure_executable_scripts(project_path: Path, tracker: StepTracker | None = None) -> None:
    """Ensure POSIX .sh scripts under .worldbuild/scripts (recursively) have execute bits (no-op on Windows)."""
    if os.name == "nt":
        return  # Windows: skip silently
    scripts_root = project_path / ".worldbuild" / "scripts"
...
def init(
    ...
    setup_lines = [
        "[cyan]Worldbuilding Kit Project Setup[/cyan]",
        "",
    ...
    tracker = StepTracker("Initialize Worldbuilding Kit Project")

    sys._worldbuild_tracker_active = True
